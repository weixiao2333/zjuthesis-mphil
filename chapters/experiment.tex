...
\section{实验结果与分析}
模型训练过程中的损失函数Loss曲线变化如图所示，可以只列两个，预训练
...
本文7个模型的实验结果如表\ref{tab:jieguo}所示,以下是对结果数据的展开分析。
...
对于测试集表现最优的bert-交互式联合识别模型型，我们进行了进一步消融分析。消融分析直白的解释就是将模型视作pipeline，在模型整体性能较好的前提下，
